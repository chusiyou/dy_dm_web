...
import com.moshi.observable.WatchedCounter;
import com.moshi.observable.Watcher1;
...
public class DyBulletScreenClient
{
	...
	static WatchedCounter watchedCounter = new WatchedCounter();
	
	static Map<String, String> liwuMap=new HashMap<String, String>();
    static Watcher1 watcher1 = new Watcher1();
    ...
    public void getServerMsg(){
		...
		try {
			...
			dataStr = new String(realBuf,"utf-8");
    ...
    private void parseServerMsg(Map<String, Object> msg){
    	...
    	if(msg.get("type") != null){
    		...
    		String divString="<div style=\"position:relative;width:300px;\">";
    		String lineString="<hr style=\"height:1px;border:none;border-top:1px solid #555555;\" />";
			//判断消息类型
			if(msg.get("type").equals("chatmsg")){//弹幕消息
				
				tempString=divString+"<font size='3' color='blue'>"+msg.get("nn")+":"+"</font>"+msg.get("txt")+"</div>";
				//System.out.println();
			} else if(msg.get("type").equals("dgb")){//赠送礼物信息
				tempString=divString+"<font size='4' color='red'>礼物消息===>"+msg.get("nn")+",赠送:"+liwuMap.get(msg.get("gfid"))+"</font>"+"</div>";;
				//logger.debug("礼物消息1111===>" + msg.toString());
			}else if(msg.get("type").equals("uenter")) {
				tempString=divString+"欢迎土豪:"+"<font size='4' color='red'>"+msg.get("nn")+"光临房间！"+"</font>";
				//logger.debug("用户进房通知消息===>" + "欢迎土豪:"+msg.get("nn")+"光临房间！");
			}else if(msg.get("type").equals("logout")) {
				//tempString= msg.toString();
				//logger.debug("用户登出通知消息===>" + msg.toString());
			}else {
	        ...
	        watchedCounter.pushMessage(tempString+lineString);
